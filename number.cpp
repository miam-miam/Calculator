...
Number Number::operator+(const Number N1) const
{
    try
    {
        ...
        if (type == POWER_TYPE && N1.type == POWER_TYPE && power.exponent == N1.power.exponent
            && power.base == N1.power.base)
        {
...
Number Number::operator*(const Number N1) const
{
    try
    {
        ...
        if (type == POWER_TYPE && N1.type == POWER_TYPE)
        {
            ...
            {
                return Number(power.multiplicand * N1.power.multiplicand,
                              power.base,
...
Number Number::operator/(const Number N1) const
{
    ...
    try
    {
        ...
        if (type == FRACTION_TYPE && N1.type == FRACTION_TYPE)
        {
            const auto numerator = fraction.integer * fraction.denominator * N1.fraction.denominator
                + N1.fraction.denominator * fraction.numerator;
            const auto denominator = N1.fraction.denominator * fraction.denominator * N1.fraction.integer
...
bool Number::checkIfZero(Number) const
{
    switch (type)
    {
        case INTEGER_TYPE:
        ...
        case FRACTION_TYPE:
        {
            return fraction.integer == 0 && fraction.numerator == 0;
        }
        case DOUBLE_TYPE:
        {
            return double_num == 0;
        ...
        case POWER_TYPE:
        {
            return power.multiplicand.integer == 0 && power.multiplicand.numerator == 0;
        }
    }
}
...
std::ostream &operator<<(std::ostream &Strm, const Number &N1)
{
    ...
    {
        Strm << int64_t(N1.fraction.integer) << "+" << int64_t(N1.fraction.numerator) << "/"
             << int64_t(N1.fraction.denominator);
    }
    ...
    {
        Strm << "(" << int64_t(N1.power.multiplicand.integer) << "+" << int64_t(N1.power.multiplicand.numerator) << "/"
             << int64_t(N1.power.multiplicand.denominator) << ")*(" << int64_t(N1.power.base.numerator) << "/"
             << int64_t(N1.power.base.denominator) << ")^(" << int64_t(N1.power.exponent.numerator) << "/"
             << int64_t(N1.power.exponent.denominator) << ")";
    }
    else
    {
        assert(0);
